...
import type { StorageEntry, CookieMetadata, SnapshotDiff, StorageSnapshot } from './types';
...
const NETSCAPE_HEADER = [
  ...
  '# This file was generated by Cookie Monster. Edit at your own risk.',
  '',
].join('\n');

/**
 * Export cookie entries to the Netscape cookies.txt format.
 * Non-cookie entries are silently skipped.
 */
export function exportToNetscape(entries: StorageEntry[]): string {
  const lines = [NETSCAPE_HEADER];

  for (const entry of entries) {
    if (entry.type !== 'cookie') continue;

    const meta = entry.metadata as CookieMetadata;
    const domain = entry.domain;
    const flag = domain.startsWith('.') && !meta.hostOnly ? 'TRUE' : 'FALSE';
    const path = meta.path || '/';
    const secure = meta.secure ? 'TRUE' : 'FALSE';
    const expiration = meta.expirationDate ? String(Math.floor(meta.expirationDate)) : '0';
    const name = entry.key;
    const value = entry.value;

    const outputDomain = meta.httpOnly ? `#HttpOnly_${domain}` : domain;

    lines.push([outputDomain, flag, path, secure, expiration, name, value].join('\t'));
  }
